...
{	
	...
	AddrInterval += ILC->SingleIntBufferWidth * ILC->SingleBufferDepth;
	ILC->PIDError = (volatile int *) pushm + 8 + AddrInterval;
	F_isClearing = 1;
	ILC_Clear(ILC);
	return 0;
}
...
int ILC_Clear(struct Para *ILC)
{
	if(F_isClearing == 1)
	{
		...
		memset(ILC->PIDError, 0, ILC->SingleBufferDepth * ILC->SingleDataBufferWidth);
		ILC->CurrentFF = 0;
		ILC->TrajRunNums = 0;
		ILC->CurTrajLength = 0;
		ILC->CurTrajCounter = 0;
...
int ILC_SaveError(struct Para *ILC,double e)
{
	if(F_isRunning)
	{
		...
		ILC->PIDError[ILC->CurTrajCounter] =  e; 
		ILC->CurTrajCounter ++;
...
/*************************
  计算下次迭代控制量
*************************/
int ILC_NextFF(struct Para *ILC, int Bound)
{
	static int i = 0;
	double ErrorTemp[CurTrajLength];

	//if(ILC->TrajRunNums > 1) 
	//{	//
		
		ILC_FiltFilt(ILC, ILC->Error, 0, Bound, 1);
		ILC_FiltFilt(ILC, ILC->PIDError, Bound, CurTrajLength, 2);
		for(i=0; i < CurTrajLength; i++)
		{
			if(i <= Bound)
			{
				ErrorTemp[i] = ILC->Error[i];
			}
			else
			{
				ErrorTemp[i] = ILC->PIDError[i];
			}
		}
		for(i=1; i < Bound; i++)  
		{
			ILC->LastFF[i] +=  ILC->kp * ErrorTemp[i] + ILC->kd * ( ErrorTemp[i] - ErrorTemp[i-1] );
		}
		ILC->LastFF[0] += ILC->kp * ErrorTemp[0] + ILC->kd * ErrorTemp[0];  //
...
/*************************
  滤波
*************************/
int ILC_FiltFilt(struct Para *ILC, volatile int *pp,  int Start, int DataLength ,int flag)
{
	float LP_a[3];
	float LP_b[3];
	if(flag == 1)//Fc
	{
		...
		LP_b[1] = 0.6553;
		LP_b[2] = 0.3276;	
		//LP_a[3] = {1, 0.1357, 0.1748};
//		LP_b[3] = {0.3276, 0.6553, 0.3276};		
	}
	else if(flag == 2)//Fc=20Hz
	{
		LP_a[0] = 1;
		LP_a[1] = -1.9213;
		LP_a[2] = 0.9243;
		...
		LP_b[1] = 0.0015;
		LP_b[2] = 0.0007;
		//LP_a[3] = {1, -1.9213, 0.9243};
//   		LP_b[3] = {0.0007, 0.0015, 0.0007};  		
	}

	int i;
	for(i = Start; i < DataLength + 10; i++)
	{
		ILC->Tmp1[i] = 0;
		ILC->Tmp2[i] = 0;
		ILC->Tmp3[i] = 0;
...
/*************************
  迭代学习控制器
*************************/
double mainPID(struct Para *ILC , double mye , double v)
{
	double u, Integrator;
	u = kp * mye - kd * v;
	Integrator += ki * mye;
...
double mainILC(struct MotorData *Mptr)
{
	...
	{	
		...
		MainIBound = (int)(T / myServoPeriod);//0.9
		...
		if(F_isRunning)		
		{
			ILC->CurrentFF += mainPID(ILC , mye , Mptr->ActVel);
			u = ILC->CurrentFF;
		}
		else
		{
			u = mainPID(ILC , mye , Mptr->ActVel);
		}

		
		if(maini == 700)
		{
			ILC_NextFF( ILC , MainIBound);
		}
		if(u>1000) 
		{
			u = 1000;
