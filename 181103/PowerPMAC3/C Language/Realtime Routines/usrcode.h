...
#define ITERPERIOD 400//迭代周期 
#define DESPOSITION 100000 //目标位置 
#define KP 0.1//
#define KD 50//
#define KI 0.1//
#define KP1 3//
...
float Step = 112500, T = ITERPERIOD;  //步长cts//和步进时间s
#define     a3       10*Step/(T*T*T)
#define     a4       -15*Step/(T*T*T*T)
#define     a5       6*Step/(T*T*T*T*T)    //轨迹规划用

struct ILCSave{
	double U[ITERPERIOD];
	double PreError[ITERPERIOD];
	double DesPosition;
	double Kd;
	double Kp;
	double Ki;
...
extern int ILC_Init(struct ILCSave *ILCData);
extern int ILC_Save_Iter_Data(struct ILCSave *ILCData , double CurError);
extern double Derivative(double e1, double e2);
extern double Integrate(double e1, double e2);
extern double ILC_Iteration(struct ILCSave *ILCData);
//extern double PIDController(struct ILCSave *ILCData , int CurCount);
extern double PIDControl(struct MotorData *Mptr);
extern double *Invertfilt(double Error[] , int length);
extern double *LowPass(double temp[] , int length)

double ILC_Main(struct MotorData *Mptr);
EXPORT_SYMBOL(ILC_Main);
